...
TextRenderer::~TextRenderer() { UnloadFont(textFont); }
...
void TextRenderer::render(ECS::World *world) {
  for (auto ent : world->each<TextComponent>()) {
    ...
    DrawTextEx(textFont, pchar, textComponent->position, textFont.baseSize, 1,
               WHITE);
  }
}
  ...
  world->each<GameTextComponent>(
      [&](ECS::Entity *entity,
        ...
        if (!gameTextComponent->destroy()) {
          ...
          DrawTextEx(textFont, pchar, gameTextComponent->position, 12, 1,
                     WHITE);
        }
      });
}
