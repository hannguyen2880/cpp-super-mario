...
SoundSystem::SoundSystem() { audioManager_ = new AudioManager(); }

SoundSystem::~SoundSystem() { delete audioManager_; }
...
void SoundSystem::receive(World *world, const SoundEvent &event) {
  ...
  if (doesSoundStopMusic(event.soundId))
    audioManager_->stopCurrentMusic();
}
